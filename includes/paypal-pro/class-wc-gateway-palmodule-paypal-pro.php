...
class WC_Gateway_Palmodule_PayPal_Pro extends WC_Payment_Gateway_CC {
    ...
    public function __construct() {
        ...
        $this->method_title = __('PayPal Pro', 'woo-paypal-pro-payment');
        $this->method_description = __('PayPal Pro works by adding credit card fields on the checkout and then sending the details to PayPal for verification.', 'woo-paypal-pro-payment');
    ...
    public function payment_fields() {
        ...
        if ($this->testmode == true) {
            echo '<p>';
            _e('NOTICE: SANDBOX (TEST) MODE ENABLED.', 'woo-paypal-pro-payment');
            echo '<br />';
            _e('For testing purposes you can use the card number 4916311462114485 with any CVC and a valid expiration date.', 'woo-paypal-pro-payment');
            echo '</p>';
        }
    ...
    public function validate_fields() {
        try {
            ...
            if (empty($card->exp_month) || empty($card->exp_year)) {
                throw new Exception(__('Card expiration date is invalid', 'woo-paypal-pro-payment'));
            }
            if (!ctype_digit($card->cvc)) {
                throw new Exception(__('Card security code is invalid (only digits are allowed)', 'woo-paypal-pro-payment'));
            }
            ...
            if (empty($card->number) || !ctype_digit($card->number)) {
                throw new Exception(__('Card number is invalid', 'woo-paypal-pro-payment'));
            }
            return true;
        } catch (Exception $e) {
    ...
    public function init_request_api() {
        try {
            include_once( WOO_PAYPAL_PRO_PAYMENT_PLUGIN_DIR . '/includes/paypal-pro/class-wc-gateway-palmodule-paypal-pro-api-handler.php' );
