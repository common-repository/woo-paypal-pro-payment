...
class WC_Gateway_Palmodule_PayPal_Pro_API_Handler {
    ...
    public function request_do_payment($order, $card) {
        ...
        try {
            ...
            $response = wp_safe_remote_post($this->gateway_settings->testmode ? $this->gateway_settings->testurl : $this->gateway_settings->liveurl, array(
                ...
                'body' => apply_filters('woo-paypal-pro-payment_request', $post_data, $order),
                'timeout' => 70,
            ...
            if (is_wp_error($response)) {
                ...
                throw new Exception(__('There was a problem connecting to the payment gateway.', 'woo-paypal-pro-payment'));
            }
            ...
            if (empty($response['body'])) {
                ...
                throw new Exception(__('Empty Paypal response.', 'woo-paypal-pro-payment'));
            }
            parse_str($response['body'], $parsed_response);
            WC_Gateway_Palmodule_PayPal_Pro::log('Parsed Response ' . print_r($parsed_response, true));

            if (!isset($parsed_response['ACK'])) {
                throw new Exception(__('Unexpected response from PayPal.', 'woo-paypal-pro-payment'));
            }
            ...
            switch (strtolower($parsed_response['ACK'])) {
                ...
                case 'successwithwarning':
                    ...
                    if ($details && strtolower($details['PAYMENTSTATUS']) === 'pending' && strtolower($details['PENDINGREASON']) === 'authorization') {
                        ...
                        $order->update_status('on-hold', sprintf(__('PayPal Pro charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'woo-paypal-pro-payment'), $txn_id));
                        ...
                        $order->add_order_note(sprintf(__('PayPal Pro payment completed (Transaction ID: %s, Correlation ID: %s)', 'woo-paypal-pro-payment'), $txn_id, $correlation_id));
                ...
                default:
                    ...
                    $order->update_status('failed', sprintf(__('PayPal Pro payment failed (Correlation ID: %s). Payment was rejected due to an error: ', 'woo-paypal-pro-payment'), $parsed_response['CORRELATIONID']) . '(' . $parsed_response['L_ERRORCODE0'] . ') ' . '"' . $error_message . '"');
                    throw new Exception($error_message);
                    break;
            }
        } catch (Exception $e) {
            wc_add_notice('<strong>' . __('Payment error', 'woo-paypal-pro-payment') . '</strong>: ' . $e->getMessage(), 'error');
            return;
        }
    }
    ...
    public function request_process_refund($order_id, $amount = null, $reason = '') {
        ...
        if ($details && strtolower($details['PENDINGREASON']) === 'authorization') {
            $order->add_order_note(__('This order cannot be refunded due to an authorized only transaction.  Please use cancel instead.', 'woo-paypal-pro-payment'));
            ...
            throw new Exception(__('This order cannot be refunded due to an authorized only transaction.  Please use cancel instead.', 'woo-paypal-pro-payment'));
        }
        $post_data = array(
            'VERSION' => $this->gateway_settings->api_version,
            'SIGNATURE' => $this->gateway_settings->api_signature,
        ...
        switch (strtolower($parsed_response['ACK'])) {
            case 'success':
            case 'successwithwarning':
                $order->add_order_note(sprintf(__('Refunded %1$s - Refund ID: %2$s', 'woo-paypal-pro-payment'), $parsed_response['GROSSREFUNDAMT'], $parsed_response['REFUNDTRANSACTIONID']));
                return true;
            default:
